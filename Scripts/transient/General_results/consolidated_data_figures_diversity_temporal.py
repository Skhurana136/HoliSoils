...
###--------------------------------------------------------
row_plots = ['Decay_constant_10','Decay_constant_20','Decay_constant_50']
col_plots = ['DOC', 'reduced_C', 'necromass', 'oxidized_C']
fig, axes = plt.subplots(3,4,sharex=True, sharey = 'row', figsize = (10,8))
ax = axes.flatten()
for i in list(range(4)):
    subset = extr_full[extr_full.C_pool==col_plots[i]]
    for j in list(range(3)):
        axindx = j*4 + i
        low_doc = subset[subset.DOC_initial_int==2000.]
        high_doc = subset[subset.DOC_initial_int==10000.]
        ax[axindx].scatter(low_doc.carbon_species*low_doc.biomass_species,low_doc[row_plots[j]],c='orange', alpha = 0.7, label = 'C poor')
        ax[axindx].scatter(high_doc.carbon_species*high_doc.biomass_species,high_doc[row_plots[j]],c='steelblue', alpha = 0.7, label = 'C rich')
        #ax[axindx].scatter(low_doc.FD_initial,low_doc[row_plots[j]],c='orange', alpha = 0.7, label = 'C poor')
        #ax[axindx].scatter(high_doc.FD_initial,high_doc[row_plots[j]],c='steelblue', alpha = 0.7, label = 'C rich')
fig.supxlabel("Functional diversity (Variance)", fontsize = 14)
ax[0].set_title("Decay constant", fontsize = 14, pad = -200)
ax[0].set_ylabel("10% loss", fontsize = 14)
ax[4].set_ylabel("20% loss", fontsize = 14)
ax[8].set_ylabel("50% loss", fontsize = 14)
#plt.xscale("log")
handles,labels=ax[axindx].get_legend_handles_labels()
h = handles[-3:]
l = labels[-3:]
plt.figlegend(h,l,title = 'C availability', fontsize = 12, title_fontsize = 12, bbox_to_anchor=(0.35,-0.1), ncol=2, loc = "lower left", borderpad=0.)

#%%
extr_full['T20_10'] = extr_full.T_20 - extr_full.T_10
extr_full['Decay_constant_20_10'] = 1/(extr_full.T20_10*5*extr_full.T_10)
#%%
