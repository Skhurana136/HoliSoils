...
project_dir = os.path.join("C:/", "Projects", "HoliSoils","data","transient", "test")
...
def run_sims (experiment, c_n, b_n, dom_initial, seed_sim, Switch_matrix, hw):
    ...
    #dom_initial, biomass_initial = generate_random_initial_conditions (dom_n-1, bio_n, mean_dom_initial, mean_bio_initial, total_dom_initial, dom_bio_ratio_initial)
    #dom_initial = np.append(dom_initial, np.zeros((1,1))) #Creating an empty carbon necromass pool
    ...
    trial = rn(maximum_capacity=5,carbon_num = dom_n,bio_num = bio_n, carbon_input = carbon_input, necromass_distribution="necromass_pool")
