...
details_subfolder = 'carbon_3_20k_days'
c_n = 3

def run_sims (experiment, c_n, dom_n_bio_n_ratio, dom_initial, seed_sim, Switch_matrix, hw):
    np.random.seed(seed_sim)
    sim = experiment + "/c_b_ratio_"+ str(dom_n_bio_n_ratio) + "/dom_initial_" + str(dom_initial) + "/seed_" + str(seed_sim)
    ...
    initial_conditions_file = os.path.join(project_dir, "simulations","carbon_3", "simulations.h5")
    prev_sim_hfile = h5py.File(initial_conditions_file, mode = 'r')
    prev_sim_data = prev_sim_hfile[experiment]["c_b_ratio_"+str(dom_n_bio_n_ratio)]['dom_initial_'+str(dom_initial)]['seed_'+str(seed_sim)]
...
for c_b_ratio in [1/3, 1, 3]:
    ...
    for t_dom_initial in [1000,5000,10000,15000,20000]:
        ...
        print(exp_details, c_b_ratio, t_dom_initial)
        sim_id, seed_dictionary = run_sims (exp_details, c_n, c_b_ratio, t_dom_initial, seed_sim, S_witch_b_1, hfile_to_write)
    ...
    for n, baseline, activity_pc in zip([0,1,2,3],["b_2", "b_3", "b_4", "b_5"] ,[0.1, 0.3, 0.5, 0.7]):
        for label, random_seed in zip(["a", "b", "c", "d", "e"], [1,2,3,4,5]):
            ...
            for t_dom_initial in [1000,5000,10000,15000,20000]:
                exp_details = baseline + "_" + label + "_"
                print(exp_details)
                sim_id, seed_dictionary = run_sims (exp_details, c_n, c_b_ratio, t_dom_initial, seed_sim, x_s_witch, hfile_to_write)
                if seed_dictionary[sim_id]["sim_status"] == "failed":
                    failed_count +=1
                empty_dic.update(seed_dictionary)
