...
project_dir = "D:\Projects\HoliSoils\data\steady_state"
...
def run_sims (experiment, c_n, b_n, dom_initial, seed_sim, Switch_matrix, hw, details_subfolder):
    ...
    initial_conditions_file = os.path.join(project_dir, "simulations",details_subfolder[:int(-1*len(redo_string))], "simulations.h5")
...
def run_sim (random_seed_number):
    details_subfolder = 'carbon_'+str(c_n) + '_' + str(random_seed_number)+'_ip_'+str(ip) + redo_string
    ...
    for N in bio_n_series:
        ...
        for t_dom_initial in init_dom_list:
                ...
                sim_id, seed_dictionary = run_sims (exp_details, c_n, N, t_dom_initial, random_seed_number, S_witch_b_1, hfile_to_write, details_subfolder)
    ...
    for N in bio_n_series:
        ...
        for n, baseline, activity_pc in zip([0,1,2,3],["b_2", "b_3", "b_4", "b_5"] ,[0.1, 0.3, 0.5, 0.7]):
            for label, random_seed in zip(["a", "b", "c", "d", "e"], [1,2,3,4,5]):
                ...
                for t_dom_initial in init_dom_list:
                    exp_details = baseline + "_" + label + "_"
                    sim_id, seed_dictionary = run_sims (exp_details, c_n, N, t_dom_initial, random_seed_number, x_s_witch, hfile_to_write, details_subfolder)
                    if seed_dictionary[sim_id]["sim_status"] == "failed":
                        failed_count +=1
                    empty_dic.update(seed_dictionary)
